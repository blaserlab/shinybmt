      ...
      fluidRow(
        column(
          ...
          lapply(1:nrow(input_choices), function(i) {
            selectInput(
              paste0(input_choices$inputId[i], "1"),
        ...
        column(
          ...
          lapply(1:nrow(input_choices), function(i) {
            selectInput(
              paste0(input_choices$inputId[i], "2"),
              sprintf("%s:", input_choices$variable_display[i]),
              choices = c(
                'Select...' = '',
                'All' = 'All',
        ...
        div(
          ...
          actionButton("gen_tbl", "Generate Baseline Table", class =
                         'btn-primary')
        ),
    ...
    output$baseline_chrcs = render_gt({
      ...
      reactive_filtered_data() %>%
        ...
        tbl_summary(by = 'group',
                    ...
                    label = tbl_variable_list,) %>% add_p() %>% add_stat_label() %>%
        as_gt()
    })
